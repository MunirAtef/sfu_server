...
class SimpleSFUClient {
  ...
  findUserVideo(consumerId) {
    return document.getElementById(`remote_${consumerId}`);
    // if (!video) {
    //   return false;
    // }
    // return video
  ...
  async handleNewProducer({ id, username }) {
    if (id === this.localUUID || this.publishersTemp.includes(id)) return;
    this.publishersTemp.push(id)
  ...
  removeUser({ id }) {
    ...
    this.publishersTemp.filter(item => item !== id);
